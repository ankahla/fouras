...
class ArticlesApiResourceArticle extends ApiResource
{
	...
	public function getArticles()
	{
		...
		$fields = $app->input->get('fields', 0, 'STRING');
		$tag = $app->input->get('tag', 0, 'INT');
		...
		$limit	= $app->input->get('limit', 0, 'uint');
		$page	= $app->input->get('p', 1, 'uint');
		...
		$ordering = $app->input->get('sort', 'created', 'STRING'); 
		$listOrder = $app->input->get('listOrder', 'DESC', 'STRING');

		$art_obj = new ContentModelArticles(['ignore_request' => true]);

		$params = $app->getParams();
		$art_obj->setState('params', $params);
		$art_obj->setState('filter.language', JLanguageMultilang::isEnabled());
		...
		// order
		if (in_array($ordering, $allFields)) {
			$art_obj->setState('list.ordering', 'a.'.$ordering);
			$art_obj->setState('list.direction', $listOrder);
		}

		// filter by tag
		if ($tag) {
			$art_obj->setState('filter.tag', $tag);
		}
		...
		$fields = $fields ? explode(',', $fields) : $defaultFields;
		...
		foreach ($rows as $subKey => $subArray)
		{
			...
			foreach ($fields as $field) {
				...
				if ($field == 'images') {
					$data[$subKey]->$field = json_decode($subArray->$field);
				} else {
					$data[$subKey]->$field = $subArray->$field;
				}
			}
...
/*			if ($subArray->images && in_array('images', $fields))
			{
...
/*			if ($subArray->created_by)
		...
		$pagination = $art_obj->getPagination();

/*		if (count($data) > 0)
		{*/
			$result->results = $data;
			$result->total = $pagination->total;
			$result->pagesTotal = $pagination->pagesTotal;
			$result->pagesStart = $pagination->pagesStart;
			$result->pagesStop = $pagination->pagesStop;
			$result->pagesCurrent = $pagination->pagesCurrent;
			$obj->success = true;
			$obj->data = $result;
